...
public class CompoundTargetTrackerInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
		...
		if (EditorGUI.EndChangeCheck())  
		{
			...
			Undo.SetSnapshotTarget(targetObjs, targetObjs[0].GetType() + " ");
		    Undo.CreateSnapshot();
...
#if UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2
			Undo.SetSnapshotTarget(targetObjs, targetObjs[0].GetType() + " targetTrackers");
		    Undo.CreateSnapshot();	
#endif
	...
	protected void GizmoVisibilityButton(Object[] targetObjs, GUIContent content, bool show)
	{
		...
		if (pressed)
		{
			...
			Undo.SetSnapshotTarget(
				subtrackers.ToArray(),
				targetObjs[0].GetType() + " Gizmo Visibility"
			);
