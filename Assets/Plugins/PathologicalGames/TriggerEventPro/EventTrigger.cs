...
namespace PathologicalGames
{
    ...
    public class EventTrigger : AreaTargetTracker
    {
		...
		/// <summary>
		/// This is usually the same as range, but when duration is used, range may change over 
		/// time while this will remain static.
		/// </summary>
		...
		private Vector3 beforeRange = Vector3.zero;
...
//			}
		...
		protected override void OnDisable()
		{
			...
			// Originally set OnFire. Set it back to be sure of a perfect state
			this.range = this.beforeRange;
			...
			if (this.duration != 0)
			{
				...
				else
				{
					...
					while (progress < 1)
					{
						...
						this.range = this.endRange * progress;
					...
					this.range = this.endRange;  // for percision
				}
			}
