...
namespace PathologicalGames
{
    ...
    public class AreaTargetTracker : TargetTracker
    {
        ...
        public enum AREA_SHAPES { Capsule, Box, Sphere }
#else
		...
		public bool is2D
		{
			get 
			{
#if (!UNITY_4_0 && !UNITY_4_0_1 && !UNITY_4_1 && !UNITY_4_2)  // Must put full property inside else namespace issue
				int shapes = (int)(AREA_SHAPES.Box2D | AREA_SHAPES.Circle2D);
				return System.Enum.IsDefined(typeof(AREA_SHAPES), shapes);
#else
				return false;  // Always false when 2D didn't exist, prior to Unity 4.3
#endif
			}
		}			
        ...
        protected override void Awake()
        {
            ...
            areaGO.transform.parent = this.xform;
...
#else
			// This used to be handled in Area.cs but moved here to match new pattern above
        ...
        {
			// This used to be in Awake so keep it as early as possible here.			
			// Set collider state before adding Area component
			//   'areaColliderEnabledAtStart' may be set by derived types before this Awake() runs.
			this.setAreaColliderEnabled(this.area.go, this.areaColliderEnabledAtStart);			
