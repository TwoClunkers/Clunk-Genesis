...
var fxBoltSpray : GameObject;

// For the next variables, @System.NonSerialized tells Unity to not serialize the variable or show it in the inspector view.
// Very handy for organization!

@System.NonSerialized
var playJumpAnim : boolean = true; //for animating jumping

// The current global direction we want the character to move in.
@System.NonSerialized
var inputMoveDirection : Vector3 = Vector3.zero; //this is set directly by the PlayerInputController
...
private function UpdateFunction () {
	...
	if (MoveWithPlatform()) {
		...
		if (moveDistance != Vector3.zero)
			controller.Move(Vector3.Scale(moveDistance, Vector3(1.0,1.0,0.0))); //RDM: added scale to remove all changes in z axis
	...
	// We were not grounded but just landed on something
		...
		playJumpAnim = true; //RDM: just landed, get ready to play jump animation
		Network.Instantiate(fxBoltSpray, transform.position,Quaternion.identity,0); //RDM: spew bolts
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	// RDM: animate jumping
	if(inputJump && playJumpAnim){
		//TODO: jump animating will likely be handled by an animator, not the motor script    transform.FindChild("core/crawler-track").animation.Play();
		playJumpAnim = false;
	}
