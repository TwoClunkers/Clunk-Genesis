...
public class CharacterMotor : MonoBehaviour
{
	...
	{
		//RDM: force z = 1.5;
		tr.position = new Vector3 (tr.position.x, tr.position.y, 1.5f);
		...
		if(MoveWithPlatform())
		{
			...
			if(moveDistance != Vector3.zero) {
				moveDistance = Vector3.Scale(moveDistance, new Vector3(1.0f,1.0f,0.0f)); //RDM: added scale to remove all changes in z axis
	...
	private Vector3 ApplyGravityAndJumping(Vector3 velocity)
	{
		...
		if(grounded)
		{
			...
			if(jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2))
			{
				...
				//JTP this disables the "kick off the walls" action, by only jumping up.
				if(TooSteep())
					jumping.jumpDir = Vector3.Slerp(Vector3.up, Vector3.up, jumping.steepPerpAmount);
				else
					jumping.jumpDir = Vector3.Slerp(Vector3.up, Vector3.up, jumping.perpAmount);
